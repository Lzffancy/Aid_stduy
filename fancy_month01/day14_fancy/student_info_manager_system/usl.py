from bll import StudentController
from model import StudentModel
#from student_info_manager_system.deal_Exceotion import get_int  想通过导入模块来处理异常，未成功

class StudentView:
    """
        学生视图:负责处理界面逻辑
    """
#-------------menu-----------------
    def __init__(self):
        self.__controller = StudentController()

    def __display_menu(self):
        print("按1键录入学生信息")
        print("按2键显示学生信息")
        print("按3键删除学生信息")
        print("按4键修改学生信息")

    def __select_menu(self):
        item = self.__get_int("请输入选项:")
        if item == "1":
            # 生成函数快捷键:atl + 回车
            self.__input_student()
        elif item == "2":
            self.__display_students()
        elif item == "3":
            self.__delete_student()
        elif item == "4":
            self.__modify_student()
#-------action------------------------------
    def __input_student(self):
        stu = StudentModel()
        stu.name = input("请输入学生姓名:")
        stu.sex = input("请输入学生性别:")
        #stu.score = int(input("请输入学生成绩:"))
        stu.score = self.__get_int("请输入学生成绩:")
        #stu.age = int(input("请输入学生年龄:"))
        stu.score = self.__get_int("请输入学生年龄:")
        self.__controller.add_student(stu)
        print("添加成功")

    def __display_students(self):
        for item in self.__controller.list_students:
            # 需要重写学生类的__str__方法
            print(item)

    def __delete_student(self):
        sid = self.__get_int("请输入需要删除的学生编号:")
        if self.__controller.remove_student(sid):
            print("删除成功")
        else:
            print("删除失败")

    def __modify_student(self):
        stu = StudentModel()
        stu.sid = self.__get_int("请输入学生编号:")
        stu.name = input("请输入学生姓名:")
        stu.sex = input("请输入学生性别:")
        stu.score = self.__get_int("请输入学生成绩:")
        stu.age = self.__get_int("请输入学生年龄:")
        if self.__controller.update_student(stu):
            print("修改成功")
        else:
            print("修改失败")
#--------entrance------------------------------
    def main(self):
                while True:
                    self.__display_menu()
                    self.__select_menu()
#--------del_excpetion-------------------------
    def __get_int(self,message):
        while True:
            try:
                data = int(input(message))
                return data  # 数据是整数，用ｒｅｔｕｒｎ退出函数
            except:
                print("输入不是整数")