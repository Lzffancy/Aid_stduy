...
### 2. 内嵌样式
  借助于style标签，在HTML文档中嵌入CSS样式代码，可以实现***CSS样式***与***HTML标签***之间的***分离***。同时需借助于CSS选择器到HTML 中匹配元素并应用样式
  示例:
...
注意 :
  ...
  ```
### 3. 选择器的优先级
使用选择器为元素设置样式,发生样式冲突时,***主要看选择器的权重,***权重越大,优先级越高(查找元素的速度)
...
复杂选择器(***后代,子代,伪类***)最终的权重为各个选择器***权重值之和***
***群组选择器***权重以每个选择器***单独***的***权重***为准，不进行相加计算
（权重相同时，后运行的生效）
...
## 六、尺寸单位
- px 像素单位
-  % 百分比，参照父元素对应属性的值进行计算（适应屏幕）
- em ***字体尺寸***单位，参照***父元素***的字体大小计算，1em=16px
- rem**字体尺寸**单位,参照***根元素***(body)的字体大小计算，1rem=16px
## 七、颜色单位
- 英文单词：red，green，blue
- rgb(r,g,b) 使用三原色表示，每种颜色取值0~255（工业三原色）
...
1. 如果元素(如div)尺寸***大于图片尺寸***，***会自动重复平铺***，直至铺满整个元素
2. 如果元素尺寸***小于图片尺寸***，图片默认从元素左上角开始显示，***超出部分不可见***
```css
...
3. 百分比
	...
	特殊值：
		...
		50% 50%   居中显示
```
精灵图技术 ：为了减少网络请求，可以将所有的***小图标拼接在一张图片上***，一次网络请求全部得到；
借助于background-position进行背景图片位置的调整，实现显示不同的图标
